...
// Render helper mirroring App's call shape: counts + content slots. Roles are no
// longer a tab here — role administration is global (a header-launched modal), so
// this panel hosts only the sheet-scoped surfaces. The Activity tab is opt-in
// (activity slot null by default).
...
describe("GovernancePanel — no Roles tab (roles are global, not per-sheet)", () => {
  it("never renders a Roles tab — only the four sheet-scoped surfaces", () => {
    // Role data is site-wide (listRoles/listRoleApplications/listRoleGrants take
    // no sheet), so role administration moved out of this per-sheet panel into a
    // global header-launched modal. The panel must expose no Roles tab.
    renderPanel({ cr: 2, notifications: 1, delegations: 1, activity: 3, activitySlot });
    expect(screen.queryByRole("tab", { name: /roles/i })).toBeNull();

    const tabNames = screen.getAllByRole("tab").map((t) => t.textContent ?? "");
    expect(tabNames).toHaveLength(4);
    expect(tabNames.join(" ")).toMatch(/change requests/i);
    expect(tabNames.join(" ")).toMatch(/notifications/i);
    expect(tabNames.join(" ")).toMatch(/delegations/i);
    expect(tabNames.join(" ")).toMatch(/activity/i);
  });
});
